...
def add_notes(prs, hooks):
    ...
    for slide, hook in zip(prs.slides, hooks):
        slide.notes_slide.notes_text_frame.text = (
            "OLI CANON SAY — revised M09 Cloud re-check PASS.\n\n"
            + hook.strip()
        )
...
student_notes = [
    "TITLE — SAY: “To the single cycle.” Introduce M09 as a hunt for precision that outruns its statistical basis. Do not preview a later statistical method.",
    ...
    "CLOSE — Collect Note version 2. Diane still wants one number. Light hook only: someone will ask whether the deck got worse since the earlier readings. Do not preview or name the next meeting’s statistical method.",
]
...
reveal_notes = [
    ...
    "REVEAL — Show the small fatigue-test dataset and visible scatter. Keep the descriptive minimum and maximum separate from methods not yet taught.",
    ...
    "NOTE — Compare claim/check/result records. The hand-in is Note version 2 only; do not require a later method.",
    "CLOSE — Diane remains in BELIEVER mode and still wants one number. Hook only the plain question of whether the deck got worse; do not preview or name a later statistical method.",
]
